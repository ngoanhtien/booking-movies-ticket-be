...
const ShowtimeSelection: React.FC<ShowtimeSelectionProps> = ({
  ...
  const handleShowtimeClick = (
    ...
    const selectedShowtime = selectedBranch?.showtimes?.find(
      (s) => s.scheduleId === showtimeId
    );
        ...
        >
          <CardContent>
            ...
            <Box display="flex" flexWrap="wrap" gap={1} ml={4}>
              {branch.showtimes &&
                branch.showtimes.map((showtime) => {
                  // Determine room type based on the showtime info
                  const roomType =
                    showtime.roomType ||
                    (showtime.roomName?.includes("IMAX")
                      ? "IMAX"
                      : showtime.roomName?.includes("4DX")
                      ? "4DX"
                      : showtime.roomName?.includes("Premium")
                      ? "Premium"
                      : "2D");
...
const MovieDetails: React.FC = () => {
  ...
  const [showShowtimesSection, setShowShowtimesSection] =
  ...
  const posterUrl =
    movie?.posterUrl || movie?.imageSmallUrl || movie?.imageLargeUrl || "";
  const movieStatus =
          ...
          <Grid item xs={12} md={8}>
            <Box sx={{ p: { xs: 3, md: 4 } }}>
              ...
              <Box sx={{ display: "flex", alignItems: "center", mb: 3 }}>
                <Rating
                  value={movie.rating ? parseFloat(movie.rating) : 4}
                  readOnly
                  precision={0.5}
                  size="medium"
                ...
                >
                  {movie.rating ? `${movie.rating}/5.0` : "4.0/5.0"} (
                  {Math.floor(Math.random() * 1000) + 100} {t("movies.ratings")}
                  )
                </Typography>
              </Box>
                        ...
                        if (!showShowtimesSection) {
                          setTimeout(() => {
                            const showtimesSection =
                              document.getElementById("showtimes-section");
                            if (showtimesSection) {
              ...
              >
                ...
                <List>
                  {typeof movie.director === "string" ? (
                    ...
                    movie.director.map((director: string, index: number) => (
                      <ListItem
                        key={index}
                        sx={{
                          px: 1,
                ...
                src={
                  movie.trailerUrl.includes("youtube.com/embed")
                    ...
                    : `https://www.youtube.com/embed/${
                        movie.trailerUrl.split("v=")[1] || movie.trailerUrl
                      }`
                }
                title={`${title} Trailer`}
                frameBorder="0"
      ...
      >
        ...
        {isLoadingShowtimes && (
          <Box sx={{ display: "flex", justifyContent: "center", py: 4 }}>
            <CircularProgress />
            <Typography sx={{ ml: 2 }}>
              {t("loading.showtimes", "Loading showtimes...")}
            </Typography>
          </Box>
        )}
        {isErrorShowtimes && (
          ...
          >
            <Typography color="error" sx={{ textAlign: "center" }}>
              {t("errors.fetchFailedDetailed", {
                entity: t("movies.showtimes", "Showtimes"),
                message:
                  (errorShowtimes as Error)?.message ||
        ...
        {showtimesData &&
          showtimesData.branches &&
          showtimesData.branches.length > 0 &&
          !isLoadingShowtimes &&
          !isErrorShowtimes && (
            <>
              {showtimesData.branches.map(
                (branch: BranchWithShowtimes, branchIndex: number) => (
                  <Box
                    key={branch.branchId}
                    sx={{
                      mb:
                        branchIndex < showtimesData.branches.length - 1 ? 4 : 0,
                    }}
                  >
                    <Paper
                      elevation={0}
                    ...
                    >
                      ...
                      {branch.hotline && (
                        ...
                        >
                          {t("common.hotline", "Hotline")}: {branch.hotline}
                        </Typography>
                      )}
                    </Paper>
                    ...
                    {branch.showtimes && branch.showtimes.length > 0 ? (
                      <Grid container spacing={1.5} sx={{ mt: 1 }}>
                        {branch.showtimes.map((st: ShowtimeDetail) => (
                          <Grid
                            item
                            key={`${st.scheduleId}-${st.roomId}-${st.scheduleTime}`}
                          >
                            <Chip
                              label={`${st.scheduleTime} (${
                                st.roomName || t("common.unknown", "Unknown")
                              })`}
                              variant="outlined"
                              clickable
                              onClick={() =>
                                navigate(
                                  `/bookings/seat-selection/${st.scheduleId}`,
                                  {
                                    state: {
                                      branchId: branch.branchId,
                                      showtimeId: st.scheduleId,
                                      roomType:
                                        st.roomType || st.roomName || "2D",
                                      selectedDate:
                                        st.scheduleDate ||
                      ...
                      >
                        {t(
                          "movies.noShowtimesForBranchDay",
                          "No showtimes available for this branch on the selected date."
                        )}
                      </Typography>
                    )}
        ...
        {showtimesData &&
          (!showtimesData.branches || showtimesData.branches.length === 0) &&
          !isLoadingShowtimes &&
          !isErrorShowtimes && (
            <Box sx={{ mt: 2, p: 3, textAlign: "center" }}>
              <Typography variant="h6" sx={{ fontStyle: "italic" }}>
                {t(
                  "movies.noShowtimesForDay",
                  "No showtimes are currently available for this movie on the selected date."
                )}
              </Typography>
            </Box>
          )}
      ...
      {movieActors.some(
        (actor) =>
          ...
          (actor.name === "Leonardo DiCaprio" &&
            title.toLowerCase().includes("joker"))
      ) && (
  ...
  return (
    <Container maxWidth="lg" sx={{ py: { xs: 4, md: 8 } }}>
      ...
      <Box sx={{ pb: 6 }}>
        <Paper
          elevation={0}
          sx={{ pb: 2, pt: isMobile ? 2 : 0, px: { xs: 0, sm: 0 } }}
        >
          {movieContent}
        </Paper>
      </Box>
