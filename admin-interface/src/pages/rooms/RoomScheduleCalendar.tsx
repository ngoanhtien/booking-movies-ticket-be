...
const RoomScheduleCalendar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // If roomId is provided, pre-select the room and cinema
    if (roomId) {
      const roomIdNum = parseInt(roomId);
      const selectedRoom = mockRooms.find(room => room.id === roomIdNum);
      
      if (selectedRoom) {
        setSelectedRoom(roomIdNum.toString());
        setSelectedCinema(selectedRoom.cinemaId.toString());
        setRoomDetails(selectedRoom);
        
        // Filter events to only show events for this room
        mockEvents = mockEvents.filter(event => event.roomId === roomIdNum);
      }
    }
  ...
  const handleCinemaChange = (event: SelectChangeEvent) => {
    ...
    // Reset room selection if cinema changes
    setSelectedRoom('');
  };
  ...
  const handleRoomChange = (event: SelectChangeEvent) => {
    ...
    // Update events based on selected room
    updateEventsForRoom(parseInt(event.target.value));
  };

  // Filter events based on selected room
  const updateEventsForRoom = (roomIdNum: number) => {
    // If we have mock data, re-fetch with the filter
    ...
    const filteredEvents = mockEvents.filter(event => event.roomId === roomIdNum);
  ...
  const handleDateClick = (arg: any) => {
    // Only open dialog if movie is selected
    if (selectedMovie && selectedRoom) {
      ...
      alert(t('schedule.selectMovieRoom', 'Please select a movie and room first'));
    }
  };
  ...
  const handleSaveEvent = () => {
    if (!currentEvent || !selectedMovie || !selectedRoom || !selectedDate || !startTime || !endTime) {
      alert(t('schedule.fillAllFields', 'Please fill all fields'));
      return;
    }
    ...
    if (!movieObj || !roomObj) {
      alert(t('schedule.selectionError', 'Error in movie or room selection'));
      return;
    }
      ...
      <Paper elevation={3} sx={{ p: 3, mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
          ...
          <Typography variant="h5">
            {roomDetails 
              ? t('schedule.roomScheduleTitle', 'Schedule for Room: {{roomName}}', { roomName: roomDetails.name }) 
              : t('schedule.movieScheduleManagement', 'Movie Schedule Management')}
          </Typography>
        </Box>
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          <Grid item xs={12} md={4}>
            <FormControl fullWidth>
              <InputLabel id="movie-select-label">{t('schedule.movie', 'Movie')}</InputLabel>
              <Select
                labelId="movie-select-label"
                value={selectedMovie}
                onChange={handleMovieChange}
                label={t('schedule.movie', 'Movie')}
              >
                {movies.map(movie => (
                  <MenuItem key={movie.id} value={movie.id.toString()}>
                    {movie.title} ({movie.duration} {t('schedule.minutes', 'minutes')})
                  </MenuItem>
                ))}
              </Select>
            </FormControl>
          ...
          {!roomId && (
            <>
              <Grid item xs={12} md={4}>
                <FormControl fullWidth>
                  <InputLabel id="cinema-select-label">{t('schedule.cinema', 'Cinema')}</InputLabel>
                  <Select
                    labelId="cinema-select-label"
                    value={selectedCinema}
                    onChange={handleCinemaChange}
                    label={t('schedule.cinema', 'Cinema')}
                  >
                    {cinemas.map(cinema => (
                      <MenuItem key={cinema.id} value={cinema.id.toString()}>
                        {cinema.name}
                      </MenuItem>
                    ))}
                  </Select>
              ...
              <Grid item xs={12} md={4}>
                <FormControl fullWidth>
                  <InputLabel id="room-select-label">{t('schedule.room', 'Room')}</InputLabel>
                  <Select
                    labelId="room-select-label"
                    value={selectedRoom}
                    onChange={handleRoomChange}
                    label={t('schedule.room', 'Room')}
                    disabled={!selectedCinema}
                  >
                    {rooms
                      .filter(room => !selectedCinema || room.cinemaId === Number(selectedCinema))
                      .map(room => (
                        <MenuItem key={room.id} value={room.id.toString()}>
                          {room.name} ({room.capacity} {t('schedule.seats', 'seats')})
                        </MenuItem>
                      ))}
                  </Select>
                </FormControl>
          ...
          <Grid item xs={12} md={roomId ? 8 : 4}>
            <FormControl fullWidth>
              <InputLabel id="timezone-select-label">{t('schedule.timezone', 'Timezone')}</InputLabel>
              <Select
                labelId="timezone-select-label"
                value={timeZone}
                onChange={handleTimezoneChange}
                label={t('schedule.timezone', 'Timezone')}
              >
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>
          {currentEvent && currentEvent.id.startsWith('new-') 
            ? t('schedule.addSchedule', 'Add Movie Schedule') 
            : t('schedule.editSchedule', 'Edit Movie Schedule')}
        </DialogTitle>
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 1 }}>
            <Grid item xs={12} md={6}>
              <FormControl fullWidth>
                <InputLabel id="dialog-movie-label">{t('schedule.movie', 'Movie')}</InputLabel>
                <Select
            ...
            {!roomId && (
              <Grid item xs={12} md={6}>
                <FormControl fullWidth>
                  <InputLabel id="dialog-room-label">{t('schedule.room', 'Room')}</InputLabel>
                  <Select
